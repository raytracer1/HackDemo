...
async function fetchAdmin(path: string, params?: Record<string, string>): Promise<any> {
...
export default function AdminPanel() {
  ...
  useEffect(() => {
    ...
    fetchAdmin('/api/admin/check')
      .then(res => {
        console.log('[Admin] Check result:', res);
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      if (tab === 'users') {
        const res = await fetchAdmin('/api/admin/users', { page: String(page), limit: String(limit) });
        setUsers(res.users); setTotal(res.total);
      } else if (tab === 'orders') {
        const res = await fetchAdmin('/api/admin/transactions', { page: String(page), limit: String(limit) });
        setTransactions(res.transactions); setTotal(res.total);
      } else {
        const res = await fetchAdmin('/api/admin/demos', { page: String(page), limit: String(limit) });
        setDemos(res.demos); setTotal(res.total);
      }
    } catch (err: any) {
  ...
  if (checkingAuth || isAdmin === null) {
    return (
      <div className="flex min-h-screen flex-col items-center justify-center">
        <div className="h-8 w-8 animate-spin rounded-full border-2 border-gray-600 border-t-hack-primary mb-3" />
    ...
    return (
      <div className="flex min-h-screen items-center justify-center px-4">
        <div className="text-center">
          ...
          <Link href="/login" className="mt-3 inline-flex items-center gap-2 rounded-xl bg-hack-primary px-6 py-3 text-sm font-semibold text-white no-underline">Sign in</Link>
          <br />
          <Link href="/" className="mt-3 inline-flex text-hack-primary hover:underline text-sm">← Back to home</Link>
        </div>
      </div>
    );
  }
