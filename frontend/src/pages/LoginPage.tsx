...
export default function LoginPage() {
  ...
  const handleEmailSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      form.method = 'POST';
      form.action = `${API_BASE}/api/auth/signin/credentials`;
          ...
          <form onSubmit={handleEmailSubmit} className="mt-6 space-y-4">
            {mode === 'register' && (
              <div>
                ...
                <input type="text" value={name} onChange={e => setName(e.target.value)} required
                  className="w-full rounded-lg border border-gray-200 px-3 py-2.5 text-sm text-gray-900 placeholder-gray-400 focus:border-hack-primary focus:ring-1 focus:ring-hack-primary outline-none" placeholder="Your name" />
              </div>
            )}
            <div>
            ...
            <div>
              ...
              <input type="password" value={password} onChange={e => setPassword(e.target.value)} required minLength={6}
                className="w-full rounded-lg border border-gray-200 px-3 py-2.5 text-sm text-gray-900 placeholder-gray-400 focus:border-hack-primary focus:ring-1 focus:ring-hack-primary outline-none" placeholder="At least 6 characters" />
            </div>
